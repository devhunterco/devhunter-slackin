...
export default function splash({ name, org, logo, active, total, channels, iframe }){
  let div = dom('.splash',
    ...
    dom('p.status',
      active
        ? [
          dom('b.active', active), ' usuarios en linea ahora de ',
          dom('b.total', total), ' registrados.'
        ]
        : [dom('b.total', total), ' usuarios se han registrado.']
    ),
    dom('form',
      ...
      dom('input.form-item type=email placeholder=tu@correo.com '
        + (!iframe ? 'autofocus' : '')),
      dom('button.loading', 'Obtener invitación')
    ),
    !iframe && dom('p.signin',
      'ó ',
      dom(`a href=https://${org}.slack.com`, 'Ingresa'),
      '.'
    ),
    !iframe && dom('footer',
      'powered by ',
